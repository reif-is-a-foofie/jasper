...
mod tests {
    ...
    #[tokio::test]
    async fn session_start_context_persists_into_later_turn_prompts() {
        let (session, turn_context) = make_session_and_context().await;
        let startup_context = "Startup context: wimboltine stonpet";
        let startup_message: ResponseItem =
            DeveloperInstructions::new(startup_context.to_string()).into();

        session
            .record_conversation_items(&turn_context, std::slice::from_ref(&startup_message))
            .await;
        ...
        let first_assistant = ResponseItem::Message {
            id: None,
            role: "assistant".to_string(),
            content: vec![ContentItem::OutputText {
        ...
        let second_user = ResponseItem::Message {
            id: None,
            role: "user".to_string(),
            content: vec![ContentItem::InputText {
        ...
        session
            .record_into_history(
                &[first_user.clone(), first_assistant, second_user.clone()],
                &turn_context,
            )
            .await;
        ...
        assert_eq!(prompt_items[0], startup_message);
        assert_eq!(prompt_items[1], first_user);
        assert_eq!(prompt_items[3], second_user);
    }
