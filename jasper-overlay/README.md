...
- `node jasper-overlay/bin/jasper.js apps` reports connector and app requests Jasper is waiting on
...
- in the live terminal chat, Jasper now auto-surfaces installed calendar and mailbox tools from normal household prompts and should send the user to `jasper apps` when a household connector is still missing
